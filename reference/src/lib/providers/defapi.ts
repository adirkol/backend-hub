...
export class DefAPIAdapter implements ProviderAdapter {
  ...
  async submitGeneration(input: GenerationInput): Promise<ProviderSubmitResult> {
    ...
    try {
      ...
      console.log(`[DefAPI] Request body:`, JSON.stringify(requestBody, null, 2));
      ...
      switch (taskData.status) {
        ...
        case "success":
          ...
          if (imageOutputs.length === 0) {
            ...
            console.error(`[DefAPI] status_reason:`, taskData.status_reason);
  ...
  async waitForResult(
    ...
    const MAX_TASK_NOT_FOUND_RETRIES = 10; // Tasks with images can take longer to register

    // Initial delay to allow task to be registered in DefAPI's system
    // Tasks with images take longer to become queryable
